        ...
        <div class="page-header" style="height: 50px">
          ...
          <div class="header-left">
            <div style="padding-left:20px;display: flex;align-items: center">
              ...
              <span style="font-size: 20px">我的申请</span>
            </div>
          </div>
              ...
              <el-table
                ...
                <el-table-column
                  ...
                  <template  slot-scope="scope">
                    <span style="color: #47a1fb;font-size: 12px;cursor: pointer;"  @click="enterMyHaveToDoDetail(scope.row)">{{ scope.row.taskName }}</span>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="processName"
                ...
                <el-table-column
                  ...
                  label="审批状态">
                  <template slot-scope="scope">
                    <span type="text" v-if="scope.row.taskStatus == 1" style="color: #4BD863" size="small">{{scope.row.statusStr}}</span>
                    <span type="text" v-if="scope.row.taskStatus == 2" style="color: #FE9400" size="small">{{scope.row.statusStr}}</span>
                    <span type="text" v-if="scope.row.taskStatus == 3 || scope.row.taskStatus == 4" style="color: red" size="small">{{scope.row.statusStr}}</span>
                  </template>
                </el-table-column>
...
<script>
  import {getUserApproveList} from '../../../../src/api/processEngine/processEngine'
  export default {
    name: 'MyApply',
    data() {
      return {
    ...
    methods: {
      ...
      requestUserApproveList(){
        ...
        getUserApproveList(param).then(response => {
