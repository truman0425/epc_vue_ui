<template>
  <div class="app-box">
    <!-- <div class="role-title">项目角色模版管理</div> -->
    <el-row :gutter="20">
      <el-col :span="5" :xs="24" class="tree-left">
        <div>
          <p
            class="add-role" @click="handleAdd"
            v-hasPermi="['system:roleTemplate:add']"><i class="el-icon-circle-plus-outline"></i>&nbsp;添加项目角色模版</p>
          <el-input
            v-model="roleTypeSearch"
            size="small"
            prefix-icon="el-icon-search"
            clearable
            placeholder="请输入角色模版名称"/>
      ...
      <el-col :span="19" :xs="24" class="table-right">
        <el-row :gutter="10" class="role-handle">
          <el-col :span="10" :xs="24" class="handle-title"><span style="color: #ffffff">1</span>{{isSetup}}项目角色模版-{{ selestTitle }}</el-col>
          <el-col :span="14" :xs="24" class="text-right" v-show="tabIndex === '0'">
            ...
            <el-button type="primary" size="small" @click.native="updateToProject">更新</el-button>
          </el-col>
        ...
        <div style="padding: 20px;">
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="功能权限" name="second">
              <el-checkbox @change="checkAll" style="padding-left: 20px;">全选</el-checkbox>
              <el-tree
                :data="menuOptions"
                show-checkbox
                ref="menu"
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="项目角色模版名称" prop="roleName">
          <el-input v-model="form.roleName" placeholder="请输入项目角色模版名称" />
        </el-form-item>
...
<script>
import { copyRole,listRole, getRole, delRole, addRole, updateRole, exportRole, dataScope, changeRoleStatus,typeRole,removeUserFromRole,addUserFromRole,pushToProject } from "@/api/system/projectRoleTemplate";
import { treeselect as menuTreeselect, projectRoleTemplateMenuTreeselect } from "@/api/system/menu";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        roleName: [
          { required: true, message: "项目角色模版名称不能为空", trigger: "blur" }
        ],
        roleKey: [
          { required: true, message: "权限字符不能为空", trigger: "blur" }
        ],
        roleSort: [
          { required: true, message: "项目角色模版顺序不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    // 更新系统角色模版至相应项目
    updateToProject() {
      this.$confirm("是否更新所有项目?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        pushToProject(this.queryParams.roleIds).then(
          response => {
    ...
    /** 查询菜单树结构 */
    getMenuTreeselect() {
      projectRoleTemplateMenuTreeselect().then(response => {
    ...
    getRoleMenuTreeselect(roleId) {
      this.loading = true;
      projectRoleTemplateMenuTreeselect(roleId).then(response => {
    ...
    handleClick(tab, event) {
      ...
      if (tab.index === '0') {this.getRoleMenuTreeselect(this.queryParams.roleIds)}
      if (tab.index == '1') {this.handleDataScope()};
    },
    // 左侧树切换
