...
function mousePosition(ev,dom){
	if(!ev) ev=window.event;
      ...
      console.log(l+"sdfasfasdasdsdasd======");
    ...
    if(ev.pageX || ev.pageY){
      ...
      var chazhi = ev.screenX - ev.offsetX;
      console.log("-------------------------------"+chazhi);

      if (l == 296) {
        return {x:ev.pageX - 325, y:ev.pageY - 165};
      }else {
        return {x:ev.pageX - 140, y:ev.pageY - 165};
      }

    }
    return {

        x:ev.clientX + document.documentElement.scrollLeft - document.body.clientLeft - 325,
        y:ev.clientY + document.documentElement.scrollTop  - document.body.clientTop - 165
    };
