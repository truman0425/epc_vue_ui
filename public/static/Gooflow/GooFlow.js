var OperGooFlow_id = "";
//定义一个区域图类：
var headBtns_list = ["new","open","save","undo","redo","reload","print"];
...
//初始化区域图的对象
	...
	this.$HeadToolsRemarks={};//每一种结点或按钮的说明文字,JSON格式,key为类名,value为用户自定义文字说明
	...
	this.$settingEnable=property.settingEnable;//是否处于设置状态
	...
	if(property.haveHead){
		...
		for(var x=0;x<property.headBtns.length;++x){
			if(headBtns_list.indexOf(property.headBtns[x]) != -1){
				tmp+="<a href='javascript:void(0)' class='GooFlow_head_btn'><i class='ico_"+property.headBtns[x]+"'></i></a>"
			}else{
//				tmp+="<a href='javascript:void(0)' class='GooFlow_head_btn'><i data-fun='"+property.headBtns[x]+"' class='"+property.headBtns_class[property.headBtns[x]]+"'></i></a>"
				tmp+="<a href='javascript:void(0)' class='GooFlow_head_btn'><i data-fun='"+property.headBtns[x]+"' class='ico_"+property.headBtns[x]+"'></i></a>"
			}
		}
    ...
    this.lineSetting=null;
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch(tar_class){
        case "ico_back":		if(This.onBtnBackClick!=null)	This.onBtnBackClick();break;
        ...
        case "ico_print":
          ...
          break;
        default :
          var funName = 'onBtn' + $(tar).attr("data-fun") + 'Click';
          if (typeof(This[funName]) == 'function') {
            This[funName]();
            break;
          }
        case "ico_clean":	if(This.onBtnCleanClick!=null)	This.onBtnCleanClick();break;
			}
		});
	}
	var toolWidth=0;
	if(property.haveTool){
	    ...
	    if(!this.$settingEnable){
	    	this.$tool.append("<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct'><i class='ico_direct'/></a>");
	    }
		...
		//加入区域划分框工具开关按钮
		if(property.haveGroup && !this.$settingEnable){
	...
	if(this.$editable){
	  ...
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		var X,Y;
		var ev=mousePosition(e,This.$workArea[0]),t=getElCoordinate(this);
	  ...
	  this.$workArea.mousemove({inthis:this},function(e){
			...
			var ev=mousePosition(e,this),t=getElCoordinate2(this);
			var X,Y;
			X=ev.x-t.left+this.parentNode.scrollLeft;
			Y=ev.y-t.top+this.parentNode.scrollTop;
	  ...
	  if(!this.$settingEnable){
		  this.$lineMove.on("mousedown",{inthis:this},function(e){
			  ...
			  var This=e.data.inthis;
			  var ev=mousePosition(e,This.$workArea[0]),t=getElCoordinate(This.$workArea[0]);
			  var X,Y;
			  ...
			  document.onmousemove=function(e){
				  if(!e)e=window.event;
				  var ev=mousePosition(e,This.$workArea[0]);
	  ...
	  var lineOperStr = "<div class='GooFlow_line_oper' style='display:none'><i class='b_l1'></i><i class='b_l2'></i><i class='b_l3'></i><i class='b_x'></i></div>";
	  	if(this.$settingEnable){
	  		lineOperStr = "<div class='GooFlow_line_oper' style='display:none'><i class='b_ls'></i></div>";
	  	}
	  this.$lineOper=$(lineOperStr);//选定线时显示的操作框
...
GooFlow.prototype={
	...
	setHeadToolsRemarks:function(remark){
		...
		this.$head.children("a").each(function(){
			this.title=remark[$(this).children(":first").attr("class").replace("ico_","")];
		});
		this.$HeadToolsRemarks=remark;
	},
	...
	addNode:function(id,json){
	  ...
	  console.log(JSON.stringify(json)+"xixixigagagagagga")
		...
		var settingrs_closeDivClass = "<div class='rs_close'></div>";
		//设置模式下不允许调整线条
    // 开始节点和结束节点不发起条件设置
    if(json.type == 'task'){
      if(this.$settingEnable){
        settingrs_closeDivClass = "<div class='rs_seeting'></div>";
//			  settingrs_closeDivClass = "<div class='rs_seeting'></div><div class='rs_modify'></div>";
      }
    }
		...
		if(json.type.indexOf(" round")<0){
			...
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div>"+ settingrs_closeDivClass +"</div></div>");
		}
		else{

			json.width=26;json.height=26;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td></tr></table><div  style='display:none'>"+ settingrs_closeDivClass +"</div><div class='span'>"+json.name+"</div></div>");
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mousedown",{inthis:this},function(e){
			...
			var This=e.data.inthis;
			if(This.$nowType!="direct")	return;
			var ev=mousePosition(e,This.$workArea[0]),t=getElCoordinate2(This.$workArea[0]);
			var X,Y;
			X=ev.x-t.left+This.$workArea[0].parentNode.scrollLeft;
			Y=ev.y-t.top+This.$workArea[0].parentNode.scrollTop;
      ...
      // console.log(JSON.stringify(this.$workArea)+"safasfdasd")

      this.$workArea.delegate(".rs_seeting","click",{inthis:this},function(e){
        if(!e)e=window.event;
  //			e.data.inthis.delNode(e.data.inthis.$focus);
        //节点设置事件
        $nodeSetting(e.data.inthis.$focus, this.$id,e);
...
//		this.$workArea.delegate(".rs_modify","click",{inthis:this},function(e){
//			if(!e)e=window.event;
//			//节点属性调整事件
//			$nodeModify(this.$focus);
//			return false;
//		});
	  }
	},
...
//节点设置事件
function $nodeSetting(nodeid, divid, e){

  var This = e.data.inthis;
  This.nodeSetting(nodeid)

  // console.log("safsdfasdfa")
	// OperGooFlow_id = divid;
	// $.modal.open("审批节点设置", "/workflow/process/nodeSetting/"+nodeid, '800', '500');
};

//线条分支条件设置事件
function $lineSetting(lineid, divid,e){
  var This = e.data.inthis;
  This.lineSetting(lineid)
	// OperGooFlow_id = divid;
	// $.modal.open("审批线条设置", "/workflow/process/lineSetting/"+lineid, '800', '500');
};

//标记已设置节点
function markItemHasbeenSet(jsondata, obj){
	for(var key in jsondata.nodes){
		if(jsondata.nodes[key] != undefined && !testIsNull(jsondata.nodes[key].nodeObj.isFixedApprover)){
			obj.markItem(key,'node',true);
		}
	}
	for(var key in jsondata.lines){
		if(jsondata.lines[key] != undefined && !testIsNull(jsondata.lines[key].nodeObj.filterVal)){
			obj.markItem(key,'line',true);
		}
	}
}

// 判断字符串是否为空
function testIsNull(str) {
	if (str == "" || str == null) {
		return true;
	} else {
		return false;
